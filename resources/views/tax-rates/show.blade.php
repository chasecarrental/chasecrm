    ...
    <div class="card">
        ...
        <div class="card-body">
            <h3 class="mb-3"> {{ $taxRate->name }} <span class="float-right">
                <a type="button" class="btn btn-outline-secondary btn-sm" href="#" onclick="loadContent('{{ url(route('laravel-crm.tax-rates.index')) }}')"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('laravel-crm::lang.back_to_tax_rates')) }}</a> | 
                @can('edit crm tax rates')
                <a href="#" onclick="loadContent('{{ url(route('laravel-crm.tax-rates.edit', $taxRate)) }}')" type="button" class="btn btn-outline-secondary btn-sm"><span class="fa fa-edit" aria-hidden="true"></span></a>
                @endcan
