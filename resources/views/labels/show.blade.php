    ...
    <div class="card">
        ...
        <div class="card-body">
            <h3 class="mb-3"> {{ $label->name }} <span class="float-right">
                <a type="button" class="btn btn-outline-secondary btn-sm" href="#" onclick="loadContent('{{ url(route('laravel-crm.labels.index')) }}')"><span class="fa fa-angle-double-left"></span> {{ ucfirst(__('laravel-crm::lang.back_to_labels')) }}</a> | 
                @can('edit crm labels')
                <a href="#" onclick="loadContent('{{ url(route('laravel-crm.labels.edit', $label)) }}')" type="button" class="btn btn-outline-secondary btn-sm"><span class="fa fa-edit" aria-hidden="true"></span></a>
                @endcan
