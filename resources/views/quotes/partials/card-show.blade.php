    ...
    @component('laravel-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                ...
                @if(! $quote->order)
                    @livewire('send-quote',[ 'quote' => $quote ])
                @endif
                @can('edit crm quotes')
                    @if(!$quote->accepted_at && !$quote->rejected_at)
                        <a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.quotes.accept', $quote) }}')" class="btn btn-success btn-sm">{{ ucfirst(__('laravel-crm::lang.mark_as_accepted')) }}</a>
                        <a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.quotes.reject', $quote) }}')" class="btn btn-danger btn-sm">{{ ucfirst(__('laravel-crm::lang.mark_as_rejected')) }}</a>
                ...
                @endcan
                @can('view crm quotes')
                    <a class="btn btn-outline-secondary btn-sm" href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.quotes.download', $quote) }}')">{{ ucfirst(__('laravel-crm::lang.download')) }}</a>
                @endcan
                @include('laravel-crm::partials.navs.activities', [ 'orders' => $orders ]) |
                @can('edit crm quotes')
                    @if(! $quote->accepted_at)
                        <a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.quotes.edit', $quote) }}')" class="btn btn-outline-secondary btn-sm"><span class="fa fa-edit" aria-hidden="true"></span></a>
                    @endif
                @endcan
                @can('delete crm quotes')
                    <form action="{{ route('laravel-crm.quotes.destroy', $quote) }}" method="POST" class="form-check-inline mr-0 form-delete-button" onsubmit="submitFormCrm(event, 'deleteQuoteForm_{{ $quote->id }}', '{{ route('laravel-crm.quotes.destroy', $quote) }}', 'Quote eliminado exitosamente', '{{ route('laravel-crm.quotes.index') }}')">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
        ...
        <div class="row card-show card-fa-w30">
            <div class="col-sm-6 border-right">
                ...
                <dl class="row">
                    ...
                    <dd class="col-sm-9">@include('laravel-crm::partials.labels', [ 'labels' => $quote->labels ])</dd>
                ...
                <hr />
                <p><span class="fa fa-address-card" aria-hidden="true"></span> @if($quote->client)<a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.clients.show', $quote->client) }}')">{{ $quote->client->name }}</a>@endif </p>
                <h6 class="mt-4 text-uppercase">{{ ucfirst(__('laravel-crm::lang.organization')) }}</h6>
                <hr />
                <p><span class="fa fa-building" aria-hidden="true"></span> @if($quote->organisation)<a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.organisations.show', $quote->organisation) }}')">{{ $quote->organisation->name }}</a>@endif</p>
                ...
                <hr />
                <p><span class="fa fa-user" aria-hidden="true"></span> @if($quote->person)<a href="javascript:void(0)" onclick="loadContent('{{ route('laravel-crm.people.show', $quote->person) }}')">{{ $quote->person->name }}</a>@endif </p>
                @isset($email)
