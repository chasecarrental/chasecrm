    ...
    @component('laravel-crm::components.card-header')
        ...
        @slot('actions')
            <span class="float-right">
                ...
                <form id="deleteUserForm_{{ $user->id }}" onsubmit="submitFormCrm(event, 'deleteUserForm_{{ $user->id }}', '{{ route('laravel-crm.users.destroy', $user) }}', '{{ __('User deleted successfully!') }}', '{{ route('laravel-crm.users.index') }}')"   method="POST" class="form-check-inline mr-0 form-delete-button">
                    {{ method_field('DELETE') }}
                     {{ csrf_field() }}
